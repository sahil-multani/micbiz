		...
		<div class="col-sm-12 " >
		  <div class="iq-card" >
			...
			<div class="iq-card-body" >
			  <p >insert a new album name to display it to users</p >

			  <form  method="post" action="" autocomplete="off">
				<div class="form-group" >
				  <label for="email text-capitalize" >album name :</label >
				  <input type="text" class="form-control" id="email1" tabindex="1" name="pname">
					<div class="error">album alredy exist !</div>
				</div >
...
<?php if (isset($_POST['pname'])) {
    $pname = $_POST['pname'];
    $sql = "insert into album values (null,'{$pname}')";
    if (!mysqli_query($conn, $sql)) {
        echo mysqli_errno($conn);
        if (mysqli_errno($conn) == 1062) {
            echo "<script>$('.error').show()
toastr['error']( 'album already exist !')</script>";
        }
    } else {
        echo "<script>toastr['info']( 'album created !')</script>";
    }
